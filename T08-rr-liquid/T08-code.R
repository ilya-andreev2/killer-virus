...
set.seed(31112)
vec <- sample(c(paste_expand("STD-", 1:16),
                paste_expand(24:39, "-", 1:5)
                ), 96, replace = FALSE)
t(matrix(vec, nrow = 8,
         dimnames = list(c(paste0("Column ", 3:10)),
                         c(paste_expand("Plate ", 1:2, " ",
                                        c("B", "C", "D", "E", "F", "G")))
                         )))

vec_std <- paste0(sprintf("%.1f", round(seq(160, 0, length.out = 16), 1)), ":",
                  sprintf("%.1f", round(seq(0, 160, length.out = 16), 1), 1))
mat_std <- matrix(vec_std, nrow = 1, byrow = TRUE,
                   dimnames = list(NULL, paste_expand("STD-", 1:16)))

file_names <- as.list(paste0("T08_04092021/",
                             c("1-NT.CSV", "2-NT.CSV", "1-T.CSV", "2-T.CSV")))
...
system("more T08_04092021/readme.info") # plate map
...
s[, "plot_index" := match(tstrsplit(strain, "-")[[1]],
                          c("26", "24", "37", "33", "29", "36", "35", "25",
                            "31", "28", "34", "39", "32", "38", "27", "30"))]
...
# ANOVA
model <- aov(pheno_fAUC ~ strain, data = s_stat); summary(model)

# Tukey's HSD post-hoc test
TukeyHSD(model)

# Dunnett's post-hoc test with resistant BY as control
