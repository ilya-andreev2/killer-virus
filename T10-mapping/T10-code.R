...
prob_median <- function(x) {
  if(length(x) %% 2 == 1) {return(median(x))}
  else {return(x[sample(c(length(x) / 2, length(x) / 2 + 1), 1)])}
}
...
match_ind <- match(sapply(rownames(g), function(x) {return(tstrsplit(x, split=":")[[1]])}), colnames(pheno.K28))
...
ggsave("~/Desktop/Paper/Figure 2/boxplot_jitter.pdf", device = 'pdf', width = 5, height = 6.3)
...
len_b <- length(breaks)
mid_breaks <- vector("numeric", len_b - 1); for (i in 1:(len_b-1)) {mid_breaks[i] <- (breaks[i] + breaks[i+1])/2}
...
ggsave('~/Desktop/Paper/Figure 2/Manhattan_Plot.pdf', device = 'pdf', width = 6, height = 3.5) # export
